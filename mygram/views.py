...
@app.route('/')
def index():
    images = Image.query.order_by(Image.id).limit(10).all()
...
def profile(user_id):
    ...
    if user==None:
        return redirect('/')
    paginate = Image.query.filter_by(user_id=user_id).order_by(Image.id.desc()).paginate(page=1,per_page=3,error_out=False)
...
def user_images(user_id,page,per_page):
    ...
    images = []
    for image in paginate.items:
        imgvo = {'id':image.id,'url':image.url,'comment_content':len(image.comments)}
        images.append(imgvo)
    map['images']=images
    return json.dumps(map)
...
def login():
    ...
    if m.hexdigest()!=user.password:
        return redirect_with_msg('/reglogin', u'用户名密码错误', 'reglogin')
    login_user(user)
    next = request.values.get('next')
    print(next)
    if next!=0 and next.startswith('/'):
...
def reg():
    ...
    salt = '.'.join(random.sample('asdfsafdsagfgfdadasfg1213',5))
    m = hashlib.md5()
